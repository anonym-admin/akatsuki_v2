...
void* FParticle::CreateBasicParticleBuffer(Particle_t* pParticles, AkU32 uParticleNum)
{
	...
	if (pResourceManager->CreateDynamicDefaultBuffer(u64BytesSize,  &pUploadBuffer))
	{
		...
		{
			pDynamicDefaultBufferHandle->uDataNum = uParticleNum;
			pDynamicDefaultBufferHandle->pUploadBuffer = pUploadBuffer;
			pDynamicDefaultBufferHandle->hSRV = hSRV;
			pDynamicDefaultBufferHandle->uSizePerType = sizeof(Particle_t);
...
AkBool FParticle::CreatePipelineState()
{
	...
	tTransparencyBlendDesc.DestBlend = D3D12_BLEND_BLEND_FACTOR;
