...
FRenderShadowMap::FRenderShadowMap()
{
}
...
AkBool FRenderShadowMap::Initialize(FRenderer* pRenderer, DWORD dwMaxItemNum)
{
...
AkBool FRenderShadowMap::Add(const RenderItem_t* pItem)
{
...
DWORD FRenderShadowMap::Process(DWORD uThreadIndex, FCommandListPool* pCmdListPool, ID3D12CommandQueue* pCmdQueue, DWORD dwProcessCountPerCommandList, D3D12_CPU_DESCRIPTOR_HANDLE hDSV, const D3D12_VIEWPORT* pViewport, const D3D12_RECT* pScissorRect)
{
	...
	while (pItem = Dispatch())
	{
		...
		{
		case RENDER_ITEM_TYPE::RENDER_ITEM_TYPE_MESH_OBJ_SHADOW_MAP:
		{
			FBasicMeshObject* pMeshObj = (FBasicMeshObject*)pItem->pObjHandle;
			pMeshObj->DrawShadowMaps(uThreadIndex, pCmdList, &pItem->tMeshObjParam.mWorld);
		}
		break;
		case RENDER_ITEM_TYPE::RENDER_ITEM_TYPE_SKINNED_MESH_OBJ_SHADOW_MAP:
		{
			FSkinnedMeshObject* pMeshObj = (FSkinnedMeshObject*)pItem->pObjHandle;
			pMeshObj->DrawShadowMaps(uThreadIndex, pCmdList, &pItem->tSkinnedMeshObjParam.mWorld, pItem->tSkinnedMeshObjParam.pBonesTransform);
		}
		break;
		case RENDER_ITEM_TYPE::RENDER_ITEM_TYPE_BILLBOARD_SHADOW_MAP:
		{
			FBillboardObjects* pBillboard = (FBillboardObjects*)pItem->pObjHandle;
			pBillboard->DrawShadowMaps(uThreadIndex, pCmdList, &pItem->tBillboardParam.mWorld);
		}
		break;
		case RENDER_ITEM_TYPE::RENDER_ITEM_TYPE_TERRAIN_OBJ_SHADOW_MAP:
		{
			FTerrainObject* pTerrain = (FTerrainObject*)pItem->pObjHandle;
			// pTerrain->DrawShadowMaps(uThreadIndex, pCmdList, &pItem->tTerrianParam.mWorld);
		}
		break;
		default:
		{
...
void FRenderShadowMap::Reset()
{
	_uAllocatedSize = 0;
	_uReadBufferPos = 0;
}

void FRenderShadowMap::CleanUp()
{
	if (_pBuffer)
	{
		free(_pBuffer);
...
const RenderItem_t* FRenderShadowMap::Dispatch()
{
	const RenderItem_t* pItem = nullptr;
